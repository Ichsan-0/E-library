	...
	<?php echo $header; ?>
		...
		<!--begin: Datatable -->
								<table class="m-datatable" id="html_table" width="100%">
    								...
    								<tbody>
									<?php $count = 1; ?>
            						<?php foreach ($pengarang as $med): ?>
                						<tr>
                    						<td><?php echo $count; ?></td>
                    						<td><?php echo $med->nama_pengarang; ?></td>
											<td><?php echo $med->tahun_lahir; ?></td>
											<td><?php echo $med->jenis_kepengarangan; ?></td>
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editMediaForm" action="<?php echo base_url('admin/master/Pengarang/updatePengarang'); ?>" method="post">
                    <div class="form-group">
                        <label for="nama_pengarang" class="form-control-label">Nama Pengarang</label>
                        <input class="form-control" id="nama_pengarang" name="nama_pengarang"></input>
                    </div>
					<div class="form-group">
                        <label for="tahun_lahir" class="form-control-label">Tahun Lahir</label>
                        <input class="form-control" id="tahun_lahir" name="tahun_lahir"></input>
                    </div>
					<div class="form-group">
                    <label for="jenis_kepengarangan" class="form-control-label">Jenis Kepengarangan</label>
                    <select class="form-control m-input" id="jenis_kepengarangan" name="jenis_kepengarangan">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" id="btnUpdatePenerbit">Update data</button>
                <input type="hidden" id="pengarangId" name="pengarangId">
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function () {
        function handleDelete(id) {
            if (confirm("Apakah Anda yakin ingin menghapus data Pengarang ini?")) {
                $.post("<?php echo base_url('admin/master/Pengarang/deletePengarang/'); ?>" + id, function (response) {
		...
		$("#btnAddPengarang").click(function (event) { // Add 'event' parameter here
        event.preventDefault(); // Prevent the default form submission
		var nama_pengarang = $("#nama").val();
		var tahun_lahir = $("#tahun").val();
		var jenis_kepengarangan = $("#jenis").val();
		var daftar_terkendali = $("#daftar").val();
		console.log("jenis_kepengarangan:", jenis_kepengarangan);
            $.ajax({
                url: "<?php echo base_url('admin/master/Pengarang/addPengarang'); ?>",
                type: "POST",
                data: {
        ...
        $(document).on('click', '.btnEditMedia', function () {
            var pengarangId = $(this).data('id');

            // AJAX request to fetch GMD data
            $.ajax({
                url: 'Pengarang/editPengarang/' + pengarangId,
                type: 'GET',
                dataType: 'json',
                success: function (data) {
                    $('#pengarangId').val(data.id);
                    $('#nama_pengarang').val(data.nama_pengarang);
                    $('#tahun_lahir').val(data.tahun_lahir);
					$('#jenis_kepengarangan').val(data.jenis_kepengarangan);
        ...
        function handleUpdate() {
            var pengarangId    = $("#pengarangId").val();
            var nama_pengarang = $("#nama_pengarang").val();
			var tahun_lahir		= $("#tahun_lahir").val();
			var jenis_kepengarangan = $("#jenis_kepengarangan").val();
			var daftar_terkendali = $("#daftar_terkendali").val();

            $.ajax({
                url: 'Pengarang/updatePengarang',
                type: "post",
                data: {
                    id: pengarangId,
                    nama_pengarang: nama_pengarang,
        ...
        $("#btnUpdatePenerbit").click(function () {
            handleUpdate();
        });
    });
</script>
