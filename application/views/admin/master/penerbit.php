	...
	<?php echo $header; ?>
		...
		<!--begin: Datatable -->
								<table class="m-datatable" id="html_table" width="100%">
    								...
    								<tbody>
									<?php $count = 1; ?>
            						<?php foreach ($penerbit as $med): ?>
                						<tr>
                    						<td><?php echo $count; ?></td>
                    						<td><?php echo $med->nama_penerbit; ?></td>
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editMediaForm" action="<?php echo base_url('admin/master/Penerbit/updatePenerbit'); ?>" method="post">
                    <div class="form-group">
                        <label for="nama_penerbit" class="form-control-label">Nama Penerbit</label>
                        <textarea class="form-control" id="nama_penerbit" name="nama_penerbit"></textarea>
                        <span class="m-form__help">Nama Penerbit tidak boleh kosong.</span>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                ...
                <input type="hidden" id="penerbitId" name="penerbitId">
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
				<div class="form-group">
                    <label for="nama_penerbit" class="form-control-label">Nama Penerbit</label>
                    <textarea class="form-control" id="nama" name="nama_penerbit"></textarea>
                    <span class="m-form__help">Nama Penerbit tidak boleh kosong.</span>
                </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary" id="btnAddPenerbit">Tambah data</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function () {
        function handleDelete(id) {
            if (confirm("Apakah Anda yakin ingin menghapus data Penerbit ini?")) {
                $.post("<?php echo base_url('admin/master/Penerbit/deletePenerbit/'); ?>" + id, function (response) {
		...
		$("#btnAddPenerbit").click(function (event) { // Add 'event' parameter here
        event.preventDefault(); // Prevent the default form submission
		var nama_penerbit = $("#nama").val();

            $.ajax({
                url: "<?php echo base_url('admin/master/Penerbit/addPenerbit'); ?>",
                type: "POST",
                data: {
        ...
        $(document).on('click', '.btnEditMedia', function () {
            ...
            $.ajax({
                url: 'Penerbit/editPenerbit/' + penerbitId,
                type: 'GET',
                dataType: 'json',
                success: function (data) {
                    $('#penerbitId').val(data.id);
        ...
        function handleUpdate() {
            var penerbitId    = $("#penerbitId").val();
            var nama_penerbit = $("#nama_penerbit").val();

            $.ajax({
                url: 'Penerbit/updatePenerbit',
                type: "post",
                data: {
                    id: penerbitId,
                    nama_penerbit: nama_penerbit
