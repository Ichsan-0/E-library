	...
	<?php echo $header; ?>
		...
		<!--begin: Datatable -->
								<table class="m-datatable" id="html_table" width="100%">
    								...
    								<tbody>
									<?php $count = 1; ?>
            						<?php foreach ($bahasa as $t): ?>
                						<tr>
                    						<td><?php echo $count; ?></td>
                    						<td><?php echo $t->kode_bahasa; ?></td>
                    						...
                    						<td>
												...
												<a class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill btnDeleteGmd" title="Delete" data-id="<?php echo $t->id; ?>"  data-nama_bahasa="<?php echo $t->nama_bahasa; ?>">
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Edit data Bahasa Dokumen</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="editTipeForm" action="<?php echo base_url('admin/referensi/Bahasa_dokumen/updateBahasa'); ?>" method="post">
				<div class="form-group">
                    <label class="form-control-label">Kode bahasa</label>
                    <input type="text" class="form-control" id="kode_bahasa" name="kode_bahasa">
                </div>
				<div class="form-group">
            ...
            <div class="modal-footer">
                ...
                <input type="hidden" id="bahasaId" name="bahasaId">
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Tambah Data Bahasa Dokumen</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
				<div class="form-group">
                    <label class="form-control-label">Kode bahasa</label>
                    <input type="text" class="form-control" id="kode" name="kode_bahasa">
                </div>
				<div class="form-group">
                    <label class="form-control-label">Nama Tempat</label>
                    <input type="text" class="form-control" id="nama" name="nama_bahasa">
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
...
<script>
    $(document).ready(function () {
        function handleDelete(id, nama_bahasa) {
            if (confirm("Apakah Anda yakin ingin menghapus data Tempat [" + nama_bahasa + "] ini?")) {
                $.post("<?php echo base_url('admin/referensi/Bahasa_dokumen/deleteBahasa/'); ?>" + id, function (response) {
		...
		var kode_bahasa = $("#kode").val();
		var nama_bahasa = $("#nama").val();

            $.ajax({
                url: "<?php echo base_url('admin/referensi/Bahasa_dokumen/addBahasa'); ?>",
                type: "POST",
                data: {
        ...
        $(document).on('click', '.btnDeleteGmd', function () {
            var id = $(this).data("id");
			var nama_bahasa = $(this).data("nama_bahasa");
            handleDelete(id, nama_bahasa);
        });

        // Additional event listener for the "Edit" buttons
        $(document).on('click', '.btnEditTipe', function () {
            var bahasaId = $(this).data('id');

            // AJAX request to fetch GMD data
            $.ajax({
                url: 'Bahasa_dokumen/editBahasa/' + bahasaId,
                type: 'GET',
                dataType: 'json',
                success: function (data) {
                    $('#bahasaId').val(data.id);
					$('#kode_bahasa').val(data.kode_bahasa);
                    $('#nama_bahasa').val(data.nama_bahasa);
        ...
        function handleUpdate() {
            var bahasaId  = $("#bahasaId").val();
            var kode_bahasa = $("#kode_bahasa").val();
			var nama_bahasa = $("#nama_bahasa").val();

            $.ajax({
                url: 'Bahasa_dokumen/updateBahasa',
                type: "post",
                data: {
                    id: bahasaId,
					kode_bahasa : kode_bahasa,
