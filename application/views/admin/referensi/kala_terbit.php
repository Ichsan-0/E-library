	...
	<?php echo $header; ?>
		...
		<!--begin: Datatable -->
								<table class="m-datatable" id="html_table" width="100%">
    								...
    								<tbody>
									<?php $count = 1; ?>
            						<?php foreach ($terbit as $t): ?>
                						<tr>
                    						<td><?php echo $count; ?></td>
                    						<td><?php echo $t->kala_terbit; ?></td>
											<td><?php echo $t->nama_bahasa; ?></td>
											<td><?php echo $t->selang_waktu; ?></td>
											<td><?php echo $t->satuan_waktu; ?></td>
                    						...
                    						<td>
												...
												<a class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill btnDeleteGmd" title="Delete" data-id="<?php echo $t->id; ?>"  data-kala_terbit="<?php echo $t->kala_terbit; ?>">
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Edit data Kala Terbit</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="editTipeForm" action="<?php echo base_url('admin/referensi/Kala_terbit/updateTerbit'); ?>" method="post">
					<div class="form-group">
						<label class="form-control-label">Kala Terbit</label>
						<input type="text" class="form-control" id="kala_terbit" name="kala_terbit">
					</div>
					<div class="form-group">
						<label class="form-control-label">Bahasa</label>
						<select class="form-control m-input" id="id_bahasa" name="id_bahasa">
					...
					<div class="form-group">
						<label class="form-control-label">Selang Waktu</label>
						<input type="number" class="form-control" id="selang_waktu" name="selang_waktu">
					</div>
					<div class="form-group">
						<label class="form-control-label">Satuan Waktu</label>
            ...
            <div class="modal-footer">
                ...
                <input type="hidden" id="terbitId" name="terbitId">
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Tambah Data Kala Terbit</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
				<div class="form-group">
                    <label class="form-control-label">Kala Terbit</label>
                    <input type="text" class="form-control" id="kala" name="kala_terbit">
                </div>
				<div class="form-group">
                    <label class="form-control-label">Bahasa</label>
                    <select class="form-control m-input" id="bahasa" name="id_bahasa">
						<?php foreach ($bahasa as $b): ?>
							<option value="<?php echo $b->id; ?>"><?php echo $b->nama_bahasa; ?></option>
						<?php endforeach; ?>
					</select>
				...
				<div class="form-group">
                    <label class="form-control-label">Selang Waktu</label>
                    <input type="number" class="form-control" id="selang" name="selang_waktu">
                </div>
				<div class="form-group">
                    <label class="form-control-label">Satuan Waktu</label>
                    <input type="text" class="form-control" id="satuan" name="satuan_waktu">
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
...
<script>
    $(document).ready(function () {
        function handleDelete(id, kala_terbit) {
            if (confirm("Apakah Anda yakin ingin menghapus data Tempat [" + kala_terbit + "] ini?")) {
                $.post("<?php echo base_url('admin/referensi/Kala_terbit/deleteTerbit/'); ?>" + id, function (response) {
		...
		var kala_terbit = $("#kala").val();
		var id_bahasa = $("#bahasa").val();
		var selang_waktu = $("#selang").val();
		var satuan_waktu = $("#satuan").val();

            $.ajax({
                url: "<?php echo base_url('admin/referensi/Kala_terbit/addTerbit'); ?>",
                type: "POST",
                data: {
        ...
        $(document).on('click', '.btnDeleteGmd', function () {
            var id = $(this).data("id");
			var kala_terbit = $(this).data("kala_terbit");
            handleDelete(id, kala_terbit);
        });

        // Additional event listener for the "Edit" buttons
        $(document).on('click', '.btnEditTipe', function () {
            var terbitId = $(this).data('id');

            // AJAX request to fetch GMD data
            $.ajax({
                url: 'Kala_terbit/editTerbit/' + terbitId,
                type: 'GET',
                dataType: 'json',
                success: function (data) {
                    $('#terbitId').val(data.id);
					$('#kala_terbit').val(data.kala_terbit);
					$('#id_bahasa').val(data.id_bahasa);
					$('#selang_waktu').val(data.selang_waktu);
        ...
        function handleUpdate() {
            var terbitId    = $("#terbitId").val();
            var kala_terbit = $("#kala_terbit").val();
			var id_bahasa   = $("#id_bahasa").val();
			var selang_waktu = $("#selang_waktu").val();
			var satuan_waktu = $("#satuan_waktu").val();

            $.ajax({
                url: 'Kala_terbit/updateTerbit',
                type: "post",
                data: {
                    id: terbitId,
					kala_terbit : kala_terbit,
