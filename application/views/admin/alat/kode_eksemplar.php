	...
	<?php echo $header; ?>
		...
		<!--begin: Datatable -->
								<table class="m-datatable" id="html_table" width="100%">
    								...
    								<tbody>
									<?php $count = 1; ?>
            						<?php foreach ($kode as $med): ?>
                						<tr>
                    						<td><?php echo $count; ?></td>
                    						<td><?php echo $med->prefiks; ?></td>
											<td><?php echo $med->sufiks; ?></td>
											<td><?php echo $med->panjang; ?></td>
											<td><?php echo $med->pola; ?></td>
                    						...
                    						<td>
												...
												<a class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill btnDeleteMedia" title="Delete" data-id="<?php echo $med->id; ?>" data-pola="<?php echo $med->pola; ?>">
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Edit Kode Eksemplar</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="editMediaForm" action="<?php echo base_url('admin/peralatan/Kode_eksemplar/updateKode'); ?>" method="post">
					<div class="form-group">
						<label class="form-control-label">Prefiks</label>
						<input type="text" class="form-control" id="prefiks" name="prefiks">
					</div>
					<div class="form-group">
						<label class="form-control-label">Sufiks</label>
						<input class="form-control" id="sufiks" name="sufiks"></input>
					</div>
					<div class="form-group">
						<label class="form-control-label">Panjang Nomer Seri</label>
						<input type="number" class="form-control" id="panjang" name="panjang"></input>
					</div>
					<div class="form-group">
                    <label class="form-control-label">Preview:</label>
					<div class="alert alert-primary" role="alert">
						<strong id="pola" name="pola"></strong>
					</div>
				</div>
            </div>
...
<script>
    $(document).ready(function () {
        function handleDelete(id, pola) {
            if (confirm("Apakah Anda yakin ingin menghapus Kode Eksemplar [" + pola + "] ini?")) {
                $.post("<?php echo base_url('admin/peralatan/Kode_eksemplar/deleteKode/'); ?>" + id, function (response) {
        ...
        $(document).on('click', '.btnDeleteMedia', function () {
            ...
            handleDelete(id, pola);
        });

        // Additional event listener for the "Edit" buttons
		$(document).on('click', '.btnEditMedia', function () {
			var kodeId = $(this).data('id');

			// AJAX request to fetch kode data
			$.ajax({
				url: 'Kode_eksemplar/editKode/' + kodeId,
				type: 'GET',
				dataType: 'json',
				success: function (data) {
					$('#kodeId').val(data.id);
					$('#prefiks').val(data.prefiks);
					$('#sufiks').val(data.sufiks);
					$('#panjang').val(data.panjang);
					$('#pola').text(data.pola);
					$('#editModal').modal('show');

					// Manually trigger the updatePreview function with the data from the edit form
					updatePreview();
				},
				error: function (xhr, status, error) {
					console.log(error);
			...
			// Additional code to handle live preview on edit modal
			$('#prefiks').on('input', updatePreview);
			$('#sufiks').on('input', updatePreview);
			$('#panjang').on('input', updatePreview);

			function updatePreview() {
				const prefiks = $('#prefiks').val();
				const sufiks = $('#sufiks').val();
				const panjang = parseInt($('#panjang').val());
				
				let nomor_seri = prefiks + "0".repeat(panjang) + sufiks;
				
				// Update the content of the <strong> tag within the alert element
				$('#pola').text(nomor_seri);
			}
		});
        ...
        function handleUpdate() {
            var kodeId    = $("#kodeId").val();
            var prefiks = $("#prefiks").val();
			var sufiks	= $("#sufiks").val();
			var panjang = $("#panjang").val();
			var pola 	= $("#pola").text(); 

            $.ajax({
                url: 'Kode_eksemplar/updateKode',
                type: "post",
                data: {
                    id: kodeId,
					prefiks : prefiks,
