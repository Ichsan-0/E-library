...
class Buku_model extends CI_Model
 {
	...
	{
		return $this->db->query('SELECT a.id,a.ISBN,a.judul,b.kategori,c.nama_penerbit,d.nama_pengarang,e.nama_rak,a.thn_terbit,a.stok,a.ket FROM buku AS a INNER JOIN tb_kategori AS b ON a.id_kategori=b.id_kategori INNER JOIN tb_penerbit AS c ON a.id_penerbit=c.id_penerbit INNER JOIN tb_pengarang AS d ON a.id_pengarang=d.id_pengarang INNER JOIN tb_rak AS e ON a.no_rak=e.no_rak'.(isset($this->wheree)?$this->wheree:''));
	}
	//query pengambilan semua data
	...
	public function buku_pinjam(){
		$query = $this->db->query("SELECT b.id, b.judul, count(td.id_buku) AS total FROM buku AS b JOIN tb_detail_pinjam AS td on b.id=td.id_buku GROUP BY td.id_buku ORDER BY total DESC LIMIT 10");
		return $query->result_array();
	}


	public function kategori_pinjam(){
		$query = $this->db->query("SELECT tb_kategori.id_kategori, tb_kategori.kategori, count(tb_detail_pinjam.id_buku) AS total FROM `tb_kategori` JOIN buku on buku.id_kategori=tb_kategori.id_kategori JOIN tb_detail_pinjam on tb_detail_pinjam.id_buku=buku.id GROUP BY tb_kategori.id_kategori ORDER BY total DESC LIMIT 10");
		return $query->result_array();
	}
	...
	public function jml_buku(){
		$query = $this->db->query("SELECT count(tb.id) AS total FROM buku tb");
		return $query->result();
	}

	public function jml_kategori(){
		$query = $this->db->query("SELECT count(tk.id_kategori) AS total FROM tb_kategori tk");
		return $query->result();
	}
